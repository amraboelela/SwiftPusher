 ...
 */
class NWSecTools: NSObject {
    /** @name Initialization */
    /** Read an identity from a PKCS #12 file (.p12) that contains a single certificate-key pair. */
    class func identity(withPKCS12Data pkcs12: Data, password: String, error: Error?) -> NWIdentityRef {
        var identities: [Any]? = try? self.identities(withPKCS12Data: pkcs12, password: password)
        if identities == nil {
            return nil
        }
        if identities?.count == 0 {
            return try? NWErrorUtil.nilWithErrorCode(kNWErrorPKCS12NoItems)!
        }
        if identities?.count > 1 {
            return try? NWErrorUtil.nilWithErrorCode(kNWErrorPKCS12MultipleItems, reason: identities?.count)!
        }
        return identities?.last!
    }
    /** Read all identities from a PKCS #12 file (.p12). */

    class func identities(withPKCS12Data pkcs12: Data, password: String, error: Error?) -> [Any] {
        if !pkcs12.length {
            return try? NWErrorUtil.nilWithErrorCode(kNWErrorPKCS12EmptyData)!
        }
        var dicts: [Any]? = try? self.allIdentities(withPKCS12Data: pkcs12, password: password)
        if dicts == nil {
            return nil
        }
        var ids: [Any] = []
        for dict: [AnyHashable: Any] in dicts {
            var identity: NWIdentityRef? = dict[(kSecImportItemIdentity as? Any)]
            if identity != nil {
...
// MARK: - Sec wrappers
    ...
    class func allIdentities(withPKCS12Data data: Data, password: String, error: Error?) -> [Any] {
        var options: [AnyHashable: Any]? = password ? [(kSecImportExportPassphrase as? Any): password] : [:]
        var items: CFArrayRef? = nil
        var status: OSStatus? = data ? SecPKCS12Import((data as? CFDataRef), (options as? CFDictionaryRef), items) : errSecParam
        var dicts: [Any] = CFBridgingRelease(items)
        if status != errSecSuccess || !items {
            switch status {
                case errSecDecode:
                    return try? NWErrorUtil.nilWithErrorCode(kNWErrorPKCS12Decode)!
                case errSecAuthFailed:
                    return try? NWErrorUtil.nilWithErrorCode(kNWErrorPKCS12AuthFailed)!
#if !TARGET_OS_IPHONE
                case errSecPkcs12VerifyFailure:
                    return try? NWErrorUtil.nilWithErrorCode(kNWErrorPKCS12Password)!
                case errSecPassphraseRequired:
                    return try? NWErrorUtil.nilWithErrorCode(kNWErrorPKCS12PasswordRequired)!
            }

            return try? NWErrorUtil.nilWithErrorCode(kNWErrorPKCS12Import, reason: status)!
        }
        return dicts
    }
